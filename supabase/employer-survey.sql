...
-- В архивной версии вопрос №18 удалён; старый столбец больше не используется.
alter table public.employer_survey_responses
  drop column if exists suggested_subjects;

alter table public.employer_survey_responses enable row level security;
revoke all on public.employer_survey_responses from public, anon, authenticated;
grant insert, select on public.employer_survey_responses to service_role;
